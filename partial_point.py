...
from utils.utils import get_dataset, remove_hidden
...
def test_zeroshot_3d_core(test_loader, validate_dataset_name, model, clip_model, tokenizer, args=None, test_data=None):
    ...
    with torch.no_grad():
        ...
        text_features = torch.tensor(np.load('data/lvis_text_features.npy'), device='cuda:0')
        end = time.time()
        ...
        for i, (pc, target, target_name, rgb) in enumerate(test_loader):
            print(f"Eval: {i}/{len(test_loader)}({round(i/len(test_loader)*100,0)}%)")
            for name in target_name:
                per_class_stats[name] += 1

            pc = remove_hidden(pc, camera)
        ...
        np.save('results/partial_points/results_top_1', per_class_correct_top1)
        np.save('results/partial_pointss/results_top_3', per_class_correct_top3)
        np.save('results/partial_points/results_top_5', per_class_correct_top5)
        np.save('results/partial_points/per_class_stats', per_class_stats)
